...
const deleteSubmissionByIndex = (array, index) => {
  /*array.splice(index);
    original, needs additional parameter for array.splice method*/
...
const deleteSubmissionByName = (array, name) => {
  ...
  if (index >= 0) {
    /*array.splice(index);
        original code, array.splice needs parameter specifying how many elements to delete*/
    array.splice(index, 1);
...
const findSubmissionByName = (array, name) => { //nested functions easier with arrows
  const query = array.find((student) => {
...
const findLowestScore = (array) => {
  ...
  array.forEach((student) => {
    if (student.score < min.score) {
      //compares each successive student in array's 'score' property with the one before it
...
addSubmission(submissions, "Jim", 45, "2021-06-25");
console.log(submissions);
console.log(filterPassing(submissions));
console.log(filter90AndAbove(submissions));
